...
func open_map(next_stage: int, choices: Array) -> void:
	...
	subtitle_label.text = "Choose your route to Stage %d" % next_stage
	hint_label.text = "Arrow Keys to navigate, Enter to confirm"
	visible = true
	_is_open = true
	_update_detail_text()
...
func _rebuild_nodes() -> void:
	...
	for idx in range(_choices.size()):
		...
		button.custom_minimum_size = Vector2(26, 18)
...
func _refresh_node_states() -> void:
	for idx in range(_button_refs.size()):
		if idx == _selected_index:
			...
			_button_refs[idx].modulate = Color(0.72, 0.72, 0.72, 1.0)
...
func _get_layout_points(count: int) -> Array[Vector2]:
	...
	if count == 2:
		return [Vector2(0.3, 0.62), Vector2(0.7, 0.38)]
	if count == 3:
		return [Vector2(0.2, 0.65), Vector2(0.5, 0.3), Vector2(0.8, 0.65)]
	if count == 4:
		return [Vector2(0.16, 0.65), Vector2(0.38, 0.35), Vector2(0.62, 0.35), Vector2(0.84, 0.65)]

	var result: Array[Vector2] = []
	for idx in range(count):
		var t: float = 0.0 if count <= 1 else float(idx) / float(count - 1)
		var y: float = 0.45 + (sin(t * PI * 2.0) * 0.18)
		result.append(Vector2(0.1 + t * 0.8, y))
	return result
